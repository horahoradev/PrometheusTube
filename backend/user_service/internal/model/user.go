...
import (
	...
	"github.com/KIRAKIRA-DOUGA/KIRAKIRA-golang-backend/user_service/errors"
...
func (m *UserModel) GetUserWithID(userID int64) (*User, error) {
	// TODO default birthdate
	sql := "SELECT id, username, email, rank, banned, coalesce(gender, '') as gender, coalesce(bio, '') as bio, birthdate as birthdate, join_date FROM users WHERE id=$1"
	var user []User
...
func (m *UserModel) GetFollowers(userID int64) ([]int64, error) {
	var users []int64
	sql := "SELECT followed_id FROM is_following WHERE following_id = $1"
	err := m.Conn.Select(&users, sql, userID)
	if err != nil {
		return nil, err
	}
...
func (m *UserModel) AddFollower(following, followed int64) error {
	sql := "INSERT INTO is_following (following_id, followed_id) VALUES ($1, $2)"
	_, err := m.Conn.Exec(sql, following, followed)
	if err != nil {
		return err
	}
...
func (m *UserModel) UpdateProfile(bio, birthdate, gender, username string, userID int64) error {
	sql := "UPDATE users SET bio=$1, birthdate=$2, gender=$3, username=$4 WHERE id = $5"
	_, err := m.Conn.Exec(sql, bio, birthdate, gender, username, userID)
	if err != nil {
		return err
	}
