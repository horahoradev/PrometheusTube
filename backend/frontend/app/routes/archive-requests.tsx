...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  return { events: events, banner: true  };
}

  
export default function Video() {
  const  { events, banner} = useLoaderData<typeof loader>();
        ...
        <TableBody>
          {events != null ? events.map((row) => (
            ...
            >
              ...
              <TableCell align="left">{row.archivedVideos + row.undownloadableVideos ==  row.currentTotalVideos ? "Synced" : "Incomplete"}</TableCell>
              <TableCell align="left">{row.userID}</TableCell>
              <TableCell align="left">{row.archivedVideos}</TableCell>
              <TableCell align="left">{row.currentTotalVideos}</TableCell>
              <TableCell align="left">{row.undownloadableVideos}</TableCell>
