...
func (j *JWTGRPCAuthenticator) GRPCAuth(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		case strings.HasPrefix(reqPath, "/api/login") || strings.HasPrefix(reqPath, "/api/register") || strings.HasPrefix(reqPath, "/api/users") || strings.HasPrefix(reqPath, "/api/logout") || strings.HasPrefix(reqPath, "/metrics") || strings.HasPrefix(reqPath, "/api/v1"):
			return next(c)
		}
